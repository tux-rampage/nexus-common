...
/**
 * Abstract implementation of a config provider Utilizing mtymek's config manager for zend expressive
 */
abstract class AbstractConfigProvider implements ConfigProviderInterface
{
    ...
    {
        $configManager = new ConfigManager($this->getProviders(), $this->getGeneratedFilePath());
    ...
    public function write()
    {
        ...
        file_put_contents($filename, '<?php return ' . var_export($config->getArrayCopy(), true) . ';');
    }
    ...
    {
        return $this->getConfig()->getArrayCopy();
    }
}
